...
servicesRoutes.get("/", async (c) => {
  try {
    ...
    const servicesResult = await db
      .prepare("SELECT * FROM services WHERE is_active = 1 ORDER BY name ASC")
      .all();
...
servicesRoutes.post("/", async (c) => {
  try {
    ...
    const existingService = await db
      .prepare("SELECT id FROM services WHERE name = ? AND is_active = 1")
      .bind(validatedData.name)
      .first();

    if (existingService) {
      return c.json(
        errorResponse("Service with this name already exists"),
        409
      );
    }
    ...
    const serviceResult = await db
      .prepare("SELECT * FROM services WHERE id = ?")
      .bind(result.meta.last_row_id?.toString())
      .first();
...
servicesRoutes.put("/:id", async (c) => {
  try {
    ...
    // Update service
    await db
      .prepare(`UPDATE services SET ${updateFields.join(", ")} WHERE id = ?`)
      .bind(...updateValues)
      .run();
...
servicesRoutes.delete("/:id", async (c) => {
  try {
    ...
    await db
      .prepare("UPDATE services SET is_active = 0, updated_at = ? WHERE id = ?")
      .bind(utils.getCurrentTimestamp(), serviceId)
      .run();
