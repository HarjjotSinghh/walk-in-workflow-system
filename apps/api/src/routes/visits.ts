...
visitsRoutes.post("/", async (c) => {
  try {
    ...
    const userCheck = await db
      ...
      .bind(validatedData.reception_id)
      .first();
    ...
    try {
      ...
      await fetch(`${baseUrl}/api/stream/broadcast`, {
        method: "POST",
    ...
    if (error && typeof error === "object" && "code" in error) {
      ...
      if (dbError.code === "SQLITE_CONSTRAINT") {
        console.error("Foreign key constraint failed:", {
          ...
          hint: "The database schema may need to be updated. Please ensure migration 0003_fix_foreign_keys.sql has been run.",
        });
        return c.json(
          errorResponse(
            "Database constraint error: The foreign key constraint failed. " +
              "This usually means the database schema needs to be updated. " +
              "Please ensure migration 0003_fix_foreign_keys.sql has been run to fix foreign key references from 'users' to 'user' table."
          ),
          400
        );
      }
...
visitsRoutes.put("/:id/status", async (c) => {
  try {
    ...
    if (
      ...
      validatedData.assigned_consultant_id
    ) {
    ...
    // Update visit
    await db
      .prepare(`UPDATE visits SET ${updateFields.join(", ")} WHERE id = ?`)
      .bind(...updateValues)
      .run();
    ...
    await utils.logAudit(
      ...
      visitId,
      "status_update",
      validatedData.pa_id || validatedData.assigned_consultant_id || "system",
      visit,
      validatedData
    );
      ...
      const eventType =
        validatedData.status === "approved"
          ...
          : validatedData.status === "denied"
            ? "visit_denied"
            : validatedData.status === "in_session"
              ? "visit_in_session"
